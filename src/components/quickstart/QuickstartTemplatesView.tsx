      ...
      <div className="px-6 pt-4 flex gap-1">
        {(["templates", "projects"] as const).map((tab) => (
          <button
            ...
            className={`px-4 py-2 text-sm font-medium rounded-t-lg transition-all duration-[120ms] ${
              activeTab === tab
          ...
          >
            ...
            {tab === "projects" && projects.length > 0 && (
              <span className="ml-1.5 text-[10px] bg-[var(--bg-surface)] text-[var(--text-secondary)] px-1.5 py-0.5 rounded-full">
                {projects.length}
              </span>
            )}
          </button>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <>
                  ...
                  <button
                    ...
                    className={`
                      ...
                      ${loadingWorkflowId === preset.id
                        ...
                        : "bg-[var(--bg-elevated)]/50 border-[var(--border-subtle)] hover:border-[var(--border-subtle)] hover:bg-[var(--bg-elevated)]"}
                      ${isLoading && loadingWorkflowId !== preset.id ? "opacity-50" : ""}
                      ${isLoading ? "cursor-not-allowed" : "cursor-pointer"}
                    `}
                  >
                    <div className={`w-8 h-8 rounded-md flex items-center justify-center flex-shrink-0 ${
                      loadingWorkflowId === preset.id ? "bg-[var(--accent-primary)]/30" : "bg-[var(--bg-surface)]/50 group-hover:bg-[var(--bg-surface)]"
                    }`}>
                      {loadingWorkflowId === preset.id ? (
                        <svg className="w-4 h-4 text-[var(--accent-primary)] animate-spin" fill="none" viewBox="0 0 24 24">
                          <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                          <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" />
                        </svg>
                      ) : (
                        <svg className="w-4 h-4 text-[var(--text-secondary)] group-hover:text-[var(--text-secondary)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                          <path strokeLinecap="round" strokeLinejoin="round" d={preset.icon} />
                        </svg>
                      )}
                    </div>
                    <div className="min-w-0">
                      <div className="text-sm font-medium text-[var(--text-primary)] truncate">{preset.name}</div>
            ...
            <div className="space-y-3">
              ...
              {isLoadingList ? (
                ...
                <p className="text-sm text-[var(--text-muted)] py-4">No community workflows available</p>
              ) : (
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                  {communityWorkflows.map((workflow) => (
                    ...
                    >
                      <div className={`w-8 h-8 rounded-md flex items-center justify-center flex-shrink-0 ${
                        loadingWorkflowId === workflow.id ? "bg-purple-500/30" : "bg-[var(--bg-surface)]/50 group-hover:bg-[var(--bg-surface)]"
                      }`}>
                        {loadingWorkflowId === workflow.id ? (
                      ...
                      <div className="min-w-0">
                        <div className="text-sm font-medium text-[var(--text-primary)] truncate">{workflow.name}</div>
          ...
          <div className="space-y-3">
            {!saveDirectoryPath ? (
              <div className="text-center py-12">
                <svg className="w-10 h-10 text-[var(--text-muted)] mx-auto mb-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 12.75V12A2.25 2.25 0 014.5 9.75h15A2.25 2.25 0 0121.75 12v.75m-8.69-6.44l-2.12-2.12a1.5 1.5 0 00-1.061-.44H4.5A2.25 2.25 0 002.25 6v12a2.25 2.25 0 002.25 2.25h15A2.25 2.25 0 0021.75 18V9a2.25 2.25 0 00-2.25-2.25h-5.379a1.5 1.5 0 01-1.06-.44z" />
                </svg>
                <p className="text-sm text-[var(--text-secondary)] mb-1">No project directory configured</p>
                <p className="text-xs text-[var(--text-muted)]">Set up a project directory in the settings to see your saved workflows here.</p>
              </div>
            ) : isLoadingProjects ? (
              ...
              <div className="text-center py-12">
                ...
                <p className="text-sm text-[var(--text-secondary)] mb-1">No saved workflows yet</p>
                <p className="text-xs text-[var(--text-muted)]">Workflows you save will appear here automatically.</p>
              </div>
            ) : (
                ...
                {projects.map((project) => (
                  ...
                  >
                    <div className={`w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 ${
                      loadingProjectFile === project.filename ? "bg-[var(--node-success)]/30" : "bg-[var(--bg-surface)]/50 group-hover:bg-[var(--bg-surface)]"
                    }`}>
                      {loadingProjectFile === project.filename ? (
                        <svg className="w-5 h-5 text-[var(--node-success)] animate-spin" fill="none" viewBox="0 0 24 24">
                          <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                          <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" />
                        </svg>
                      ) : (
                        <svg className="w-5 h-5 text-[var(--text-secondary)] group-hover:text-[var(--text-secondary)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        <span className="text-[10px] text-[var(--text-muted)]">{formatRelativeDate(project.modifiedAt)}</span>
                        <span className="text-[10px] text-[var(--text-muted)]">•</span>
                        <span className="text-[10px] text-[var(--text-muted)]">{project.nodeCount} nodes</span>
                      </div>
                    </div>
                    <svg className="w-4 h-4 text-[var(--text-muted)] group-hover:text-[var(--text-secondary)] flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        ...
        {error && (
          <div className="flex items-start gap-2 p-3 rounded-lg bg-[var(--node-error)]/10 border border-[var(--node-error)]/30">
            <svg className="w-4 h-4 text-[var(--node-error)] mt-0.5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
            </svg>
            <div className="flex-1">
              <p className="text-sm text-[var(--node-error)]">{error}</p>
              <button onClick={() => setError(null)} className="text-xs text-[var(--node-error)]/70 hover:text-[var(--node-error)] mt-1">Dismiss</button>
            </div>
          </div>
        )}
      </div>
